...
void RequestStats::getStats(TicksClock::Ticks now,
                            uint32_t* reqsLastSec, uint32_t* timeToParse, uint32_t* timeToHandleRequest, uint32_t* timeToOpenConn) const {
  uint32_t reqs_acc = 0;
  ...
  *reqsLastSec = reqs_acc;
  if(timeToParse!=NULL) {
   *timeToParse = timeToParseRequest_;
  }
  if(timeToHandleRequest!=NULL) {
    *timeToHandleRequest = timeToHandleRequest_;
  }
  if(timeToOpenConn !=NULL) { 
    *timeToOpenConn = timeToEstablishConn_;
  }
 // cout << "Time to Parse : " << timeToParseRequest_ << endl;
 // cout << "Time to Open Connection: " << timeToHandleRequest_ << endl;
 // cout << "Time to Service Request: " << timeToEstablishConn_ << endl;
 // cout << "Time to Write Output: " << timeToWriteOutput_ << endl;
