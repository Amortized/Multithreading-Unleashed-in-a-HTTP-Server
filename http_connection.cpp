...
HTTPServerConnection::HTTPServerConnection(HTTPService* service, int client_fd)
  ...
  //Calculating the time to Establish a Connection
...
//  double in_seconds = duration / TicksClock::ticksPerSecond();
  //Update time taken by this request
//  RequestStats* stats = io_service()->stats();
//  stats->updateTimeToEstablishConn(in_seconds);
...
//      end = TicksClock::getTicks();
  //    duration = start - end;
    //  double in_seconds = duration / TicksClock::ticksPerSecond();
  //    RequestStats* stats = io_service()->stats();
    //  stats->updateTimeToParse(in_seconds);
      return false;
    } else if (rc > 0) {
    //  end = TicksClock::getTicks();
    //  duration = end - start;
