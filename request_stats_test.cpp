...
TEST(Sequential, Intial) {
  ...
  uint32_t req_rate = 0;
  stats.getStats(11, &req_rate, NULL, NULL, NULL);
...
TEST(Sequential, Stale) {
  ...
  uint32_t req_rate = 0;
  stats.getStats(kTicksPerSecond+1, &req_rate, NULL, NULL, NULL);
...
TEST(Sequential, SecondSlot) {
  ...
  uint32_t req_rate = 0;
  stats.getStats(now+1, &req_rate,NULL, NULL, NULL);
  EXPECT_EQ(req_rate, 1);

  // and later, when we rolled to the next slot already
  uint64_t later = stats.ticksPerSlot()*2+1;
  stats.getStats(later, &req_rate, NULL, NULL, NULL);
  EXPECT_EQ(req_rate, 1);
}
...
TEST(Sequential, RollOver) {
  ...
  uint32_t req_rate = 0;
  stats.getStats(now, &req_rate,NULL, NULL, NULL);
