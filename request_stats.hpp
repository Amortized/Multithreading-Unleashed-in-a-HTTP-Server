...
//  void getStats(TicksClock::Ticks now, uint32_t* reqsLastSec) const;
  void getStats(TicksClock::Ticks now, uint32_t* reqsLastSec, uint32_t* timeToParse, uint32_t* timeToHandleRequest, uint32_t* timeToOpenConn) const;

  
  void updateTimeToParse(double seconds) {
    timeToParseRequest_ += seconds;
  }

  void updateTimeToHandleRequest(double seconds) {
    timeToHandleRequest_ += seconds;
  }

  void updateTimeToEstablishConn(double seconds) {
    timeToEstablishConn_ += seconds;
  }
   
  void updateTimeToWriteOutput(double seconds) {
    timeToWriteOutput_ += seconds;
  }
...
private:
  TicksClock::Ticks start;
  double timeToParseRequest_;
  double timeToHandleRequest_;
  double timeToEstablishConn_;
  double timeToWriteOutput_;
