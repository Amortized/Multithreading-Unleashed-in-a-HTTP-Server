...
class Tester {
public:
  Tester(LockFreeList<int>* list, int num_ops, int num_threads)
    : list_(list),
      num_ops_(num_ops),
...
TEST(Sequential, SimpleInsertion) {
  LockFreeList<char> l(1 /* single worker */);
...
TEST(Concurrency, InsertionThenDeletion) {
  ...
  LockFreeList<int> l(NUM_THREADS);
